...
class LibriDataset(Dataset):
    def __init__(self, split='train', patch_size = 0.1, n_predictions = 20):
        assert split in ['train', 'test'], 'Only train and test splits are implemented.'
        self.patch_size = np.around(patch_size*16000)
    ...
    def __getitem__(self, idx):
        ...
        mfcc_spectrograms = map(lambda patch: normalize(mfcc_transform(patch)), patches) # apply mfcc transform to and normalize each patch
